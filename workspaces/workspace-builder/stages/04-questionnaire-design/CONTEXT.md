...
Note: Stage 03 already seeds the world-bible prerequisite check block into setup/questionnaire.md. This stage designs only the role-specific questions that follow it.

## Inputs

| Source | File/Location | Section/Scope | Why |
|--------|--------------|---------------|-----|
| Discovery output | `../01-discovery/output/discovery-summary.md` | "User-Specific Variables" section | Variables that need onboarding questions |
...
1. Read the discovery summary's user-specific variables section
2. Scan all markdown files in the scaffolded workspace for `{{PLACEHOLDER}}` patterns. Build a complete list. Exclude any placeholders already handled by the world-bible prerequisite block.
...
5. For voice/style questions, ask for concrete examples rather than descriptions. "Give me 2-3 sentences that sound like your brand" extracts pattern-matchable rules. "Describe your voice" extracts an abstraction the agent must interpret.
...
9. If the workspace has voice/style rules with derived fields, add a two-pass process: after the agent populates rules from answers, it presents them to the user for review before finalizing.
...
13. Append the completed role-specific questions to the pre-seeded setup/questionnaire.md in the scaffolded workspace output.
...
| World-bible block preserved | The prerequisite check block seeded by Stage 03 is intact and unchanged |

## Outputs

| Artifact | Location | Format |
|----------|----------|--------|
| Completed questionnaire | `../03-scaffolding/output/[workspace-name]/setup/questionnaire.md` | Prerequisite block + flat numbered role-specific questions, fully hydrated |
